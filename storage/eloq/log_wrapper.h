...
namespace MyEloq
{
class MyEloqLogAgent : public txservice::TxLog
{
public:
  MyEloqLogAgent(const MyEloqLogAgent &rhs)= delete;

  explicit MyEloqLogAgent(const uint32_t log_group_replica_num)
      : log_agent_(), log_group_replica_num_(log_group_replica_num)
  {
  }

  ~MyEloqLogAgent()= default;

  void WriteLog(uint32_t log_group_id, brpc::Controller *cntl,
  ...
  void CheckMigrationIsFinished(
      uint32_t log_group_id, brpc::Controller *cntl,
      const ::txlog::CheckMigrationIsFinishedRequest &request,
      ::txlog::CheckMigrationIsFinishedResponse &response,
      google::protobuf::Closure &done) override
  {
    log_agent_.CheckMigrationIsFinished(log_group_id, cntl, &request,
                                        &response, &done);
  }

  ::txlog::CheckClusterScaleStatusResponse::Status
  CheckClusterScaleStatus(uint32_t log_group_id,
                          const std::string &id) override
  {
    return log_agent_.CheckClusterScaleStatus(log_group_id, id);
  }

  void UpdateCheckpointTs(uint32_t cc_node_group_id, int64_t term,
                          uint64_t checkpoint_timestamp) override
  {
    log_agent_.UpdateCheckpointTs(cc_node_group_id, term,
  ...
  void ReplayLog(uint32_t cc_node_group_id, int64_t term,
                 const std::string &source_ip, uint16_t source_port,
                 int log_group, uint64_t start_ts,
                 std::atomic<bool> &interrupt) override
  {
    log_agent_.ReplayLog(cc_node_group_id, term, source_ip, source_port,
                         log_group, start_ts, interrupt);
  }

  txservice::RecoverTxStatus RecoverTx(uint64_t tx_number, int64_t tx_term,
                                       uint64_t write_lock_ts,
                                       uint32_t cc_ng_id, int64_t cc_ng_term,
                                       const std::string &source_ip,
                                       uint16_t source_port) override
  {
    ::txlog::RecoverTxResponse_TxStatus status= log_agent_.RecoverTx(
        tx_number, tx_term, write_lock_ts, cc_ng_id, cc_ng_term, source_ip,
        source_port, GetLogGroupId(tx_number));
  ...
  uint32_t LogGroupCount() override { return log_agent_.LogGroupCount(); }
