...
inline void mono_netstr_append_uint64(String *const out_netstr,
                                      const uint64 val)
{
...
inline void mono_netstr_append_uint32(String *const out_netstr,
                                      const uint32 val)
{
  DBUG_ASSERT(out_netstr != nullptr);
...
inline void mono_netstr_append_uint16(String *const out_netstr,
                                      const uint16 val)
{
  DBUG_ASSERT(out_netstr != nullptr);
...
inline void mono_netbuf_store_uint64(uchar *const dst_netbuf, const uint64 n)
{
...
inline void mono_netbuf_store_uint32(uchar *const dst_netbuf, const uint32 n)
{
  DBUG_ASSERT(dst_netbuf != nullptr);
...
inline void mono_netbuf_store_uint16(uchar *const dst_netbuf, const uint16 n)
{
  DBUG_ASSERT(dst_netbuf != nullptr);
...
inline void mono_netbuf_store_byte(uchar *const dst_netbuf, const uchar c)
{
  DBUG_ASSERT(dst_netbuf != nullptr);
...
inline void mono_netbuf_store_index(uchar *const dst_netbuf,
                                    const uint32 number)
{
  DBUG_ASSERT(dst_netbuf != nullptr);

  mono_netbuf_store_uint32(dst_netbuf, number);
}

/*
  Basic conversion helper functions from network byte order (Big Endian) to
  host machine byte order (usually Little Endian).
*/

inline uint64 mono_netbuf_to_uint64(const uchar *const netbuf)
{
...
inline uint32 mono_netbuf_to_uint32(const uchar *const netbuf)
{
  DBUG_ASSERT(netbuf != nullptr);
...
inline uint16 mono_netbuf_to_uint16(const uchar *const netbuf)
{
  DBUG_ASSERT(netbuf != nullptr);
...
inline uchar mono_netbuf_to_byte(const uchar *const netbuf)
{
  DBUG_ASSERT(netbuf != nullptr);
...
inline uint64 mono_netbuf_read_uint64(const uchar **netbuf_ptr)
{
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint64 host_val= mono_netbuf_to_uint64(*netbuf_ptr);
...
inline uint32 mono_netbuf_read_uint32(const uchar **netbuf_ptr)
{
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint32 host_val= mono_netbuf_to_uint32(*netbuf_ptr);
...
inline uint16 mono_netbuf_read_uint16(const uchar **netbuf_ptr)
{
  DBUG_ASSERT(netbuf_ptr != nullptr);

  // Convert from network byte order (Big Endian) to host machine byte order
  // (usually Little Endian).
  const uint16 host_val= mono_netbuf_to_uint16(*netbuf_ptr);
...
class mono_string_reader
{
  const char *m_ptr;
  uint m_len;

private:
  mono_string_reader &operator=(const mono_string_reader &)= default;

public:
  mono_string_reader(const mono_string_reader &)= default;
  /* named constructor */
  static mono_string_reader read_or_empty(const MyEloq::Slice *const slice)
  {
    if (!slice)
    {
      return mono_string_reader("");
    }
    else
    {
      return mono_string_reader(slice);
    }
  }

  explicit mono_string_reader(const std::string &str)
  {
    m_len= str.length();
    if (m_len)
    {
    ...
    {
      /*
        One can a create a mono_string_reader for reading from an empty string
  ...
  explicit mono_string_reader(const MyEloq::Slice *const slice)
  {
    m_ptr= slice->data();
    m_len= slice->size();
  }
...
class mono_string_writer
{
  std::vector<uchar> m_data;

public:
  mono_string_writer(const mono_string_writer &)= delete;
  mono_string_writer &operator=(const mono_string_writer &)= delete;
  mono_string_writer()= default;
  ...
  void write_uint16(const uint val)
  {
    ...
    mono_netbuf_store_uint16(m_data.data() + size, val);
  }
  ...
  void write_uint32(const uint val)
  {
    ...
    mono_netbuf_store_uint32(m_data.data() + size, val);
  }
  ...
  void write_uint16_at(const size_t pos, const uint new_val)
  {
    ...
    mono_netbuf_store_uint16(m_data.data() + pos, new_val);
  }
  ...
  /*
    An awful hack to deallocate the buffer without relying on the
    deconstructor. This is needed to suppress valgrind errors in
    rocksdb.partition
  */
...
/*
   A helper class for writing bits into mono_string_writer.

   The class assumes (but doesn't check) that nobody tries to write
   anything to the mono_string_writer that it is writing to.
*/
class mono_bit_writer
{
  mono_string_writer *m_writer;
  uchar m_offset;

public:
  mono_bit_writer(const mono_bit_writer &)= delete;
  mono_bit_writer &operator=(const mono_bit_writer &)= delete;

  explicit mono_bit_writer(mono_string_writer *writer_arg)
...
class mono_bit_reader
{
  const uchar *m_cur;
  uchar m_offset;
  uint m_ret;
  mono_string_reader *const m_reader;

public:
  mono_bit_reader(const mono_bit_reader &)= delete;
  mono_bit_reader &operator=(const mono_bit_reader &)= delete;

  explicit mono_bit_reader(mono_string_reader *const reader)
...
template <size_t buf_length> class mono_buf_writer
{
public:
  mono_buf_writer(const mono_buf_writer &)= delete;
  mono_buf_writer &operator=(const mono_buf_writer &)= delete;
  mono_buf_writer() { reset(); }

  void write_uint32(const uint32 n)
  {
    DBUG_ASSERT(m_ptr + sizeof(n) <= m_buf.data() + buf_length);
    mono_netbuf_store_uint32(m_ptr, n);
    m_ptr+= sizeof(n);
  }
  ...
  {
    DBUG_ASSERT(m_ptr + sizeof(n) <= m_buf.data() + buf_length);
    mono_netbuf_store_uint64(m_ptr, n);
    m_ptr+= sizeof(n);
  }
  ...
  {
    DBUG_ASSERT(m_ptr + sizeof(n) <= m_buf.data() + buf_length);
    mono_netbuf_store_uint16(m_ptr, n);
    m_ptr+= sizeof(n);
  }
  ...
  void write_byte(const uchar c)
  {
    ...
    mono_netbuf_store_byte(m_ptr, c);
    m_ptr+= sizeof(c);
  }
  ...
  MyEloq::Slice to_slice() { return MyEloq::Slice(data(), size()); }
