...
#ifdef WITH_ELOQ_STORAGE_ENGINE
            ||
            /* mysql.mono_view table */
            (my_tolower(ci, name[0]) == 'm' &&
             my_tolower(ci, name[1]) == 'o' &&
             my_tolower(ci, name[2]) == 'n' &&
             my_tolower(ci, name[3]) == 'o' &&
             my_tolower(ci, name[4]) == '_' &&
             my_tolower(ci, name[5]) == 'v' &&
             my_tolower(ci, name[6]) == 'i' &&
             my_tolower(ci, name[7]) == 'e' &&
             my_tolower(ci, name[8]) == 'w')
#endif
            )
           )
         );
...
enum open_frm_error open_table_def(THD *thd, TABLE_SHARE *share, uint flags)
{
...
#ifdef WITH_ELOQ_STORAGE_ENGINE
    if ((flags & GTS_TABLE)
        // The truncate table statement not set flag GTS_USE_DISCOVERY.
        // But eloq store frm binary in remote, we need call discover to
        // fetch it.
        /* && (flags & GTS_USE_DISCOVERY) */)
    {
      ha_discover_table(thd, share);
      if (share->error == OPEN_FRM_OK)
      {
        error_given= true;
        goto err_not_open;
      }
      ...
      {
        if (share->error != OPEN_FRM_OPEN_ERROR)
        {
          goto err_not_open;
        }
        else
        {
          // share->error == OPEN_FRM_OPEN_ERROR means table not exists.
          // Next, eloq should query view storage.
        }
      }
    }
#else
  ...
  if (file < 0)
  {
    mono_errno= eloq_fetch_view(thd, share->db, share->table_name,
                                     view_frm_binary);
    if (mono_errno)
    {
      ...
      {
        my_printf_error(mono_errno, "Eloq fetch view './%s/%s' failed",
                        MYF(0), share->db.str, share->table_name.str);
        share->open_errno= OPEN_FRM_OPEN_ERROR ;
      }
      error_given= true;
      goto err_not_open;
    }
    if (view_frm_binary.length < sizeof(head))
    {
      share->error = my_errno == HA_ERR_FILE_TOO_SHORT
                        ? OPEN_FRM_CORRUPTED : OPEN_FRM_READ_ERROR;
      goto err_not_open;
    }
    memcpy(head, view_frm_binary.str, sizeof(head));
  }
  else
#endif
...
#ifdef WITH_ELOQ_STORAGE_ENGINE
      (void)length;
      share->view_def= sql_parse_prepare(&share->normalized_path,
                                         view_frm_binary, &share->mem_root, true);
#else
...
static uint enum_value_with_check_no_thd(TABLE_SHARE *share, const char *name,
                                         uint value, uint limit)
{
  if (value < limit)
    return value;
...
int TABLE_SHARE::init_from_binary_frm_no_thd(const uchar *frm_image,
                                             size_t frm_length,
                                             const uchar *par_image,
                                             size_t par_length)
{
  ...
  */
  ...
  if (!frm_image[32])				// New frm file in 3.23
  {
    ...
    share->avg_row_length= uint4korr(frm_image+34);
    share->transactional= (ha_choice)
      enum_value_with_check_no_thd(share, "transactional", frm_image[39] & 3, HA_CHOICE_MAX);
    share->page_checksum= (ha_choice)
      enum_value_with_check_no_thd(share, "page_checksum", (frm_image[39] >> 2) & 3, HA_CHOICE_MAX);
    if (((ha_choice) enum_value_with_check_no_thd(share, "sequence",
                                           (frm_image[39] >> 4) & 3,
                                           HA_CHOICE_MAX)) == HA_CHOICE_YES)
    {
      share->table_type= TABLE_TYPE_SEQUENCE;
    ...
    share->row_type= (enum row_type)
      enum_value_with_check_no_thd(share, "row_format", frm_image[40], ROW_TYPE_MAX);
  ...
  if ((n_length= uint4korr(frm_image+55)))
  {
    ...
    if (next_chunk + 2 < buff_end)
    {
      ...
      next_chunk+= str_db_type_length + 2;
		...
		share->set_use_ext_keys_flag(HTON_SUPPORTS_EXTENDED_KEYS > 0);
...
bool TABLE::index_contains_some_virtual_gcol(uint index_no) const 
{
    ...
    bool contain = false;
    KEY_PART_INFO *key_part= key_info[index_no].key_part;
    uint key_parts= key_info[index_no].user_defined_key_parts;
    for (uint k = 0; k < key_parts; k++)
    {
      if (!key_part[k].field->stored_in_db())
      {
        contain = true;
        break;
      }
