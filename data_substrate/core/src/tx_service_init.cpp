...
DEFINE_int32(collect_active_tx_ts_interval_seconds, 2,
...
DEFINE_uint32(max_standby_lag, 400000,
...
DEFINE_string(hm_bin_path, "",
              ...
              "main process");
DEFINE_uint32(deadlock_check_interval_seconds, 10,
              "Deadlock check interval in seconds");
DEFINE_bool(realtime_sampling, true,
...
DEFINE_bool(auto_redirect, false, "If redirect remote object command to remote node internally");
...
bool DataSubstrate::InitializeTxService(const INIReader &config_reader)
{
  uint64_t checkpointer_interval=
      !CheckCommandLineFlagIsDefault("checkpointer_interval")
          ...
          : config_reader.GetInteger("local", "checkpointer_interval",
                                     FLAGS_checkpointer_interval);
  ...
  uint64_t checkpointer_delay_seconds=
      !CheckCommandLineFlagIsDefault("checkpointer_delay_seconds")
          ...
          : config_reader.GetInteger("local", "checkpointer_delay_seconds",
                                     FLAGS_checkpointer_delay_seconds);
  ...
  if (CheckCommandLineFlagIsDefault(field_mem))
  {
    ...
    {
      node_memory_limit_mb= config_reader.GetInteger("local", field_mem, 0);
  ...
  tx_service_= std::make_unique<txservice::TxService>(
      catalog_factory, system_handler_, tx_service_conf,
      network_config_.node_id, network_config_.native_ng_id,
      &network_config_.ng_configs, network_config_.cluster_config_version,
  ...
  if (tx_service_->Start(
          network_config_.node_id, network_config_.native_ng_id,
          &network_config_.ng_configs, network_config_.cluster_config_version,
          &log_service_config_.txlog_ips, &log_service_config_.txlog_ports,
          &hm_ip, &hm_port, &hm_bin_path, tx_service_conf,
          std::move(log_agent), tx_path,
          network_config_.cluster_config_file_path, fork_host_manager) != 0)
  {
