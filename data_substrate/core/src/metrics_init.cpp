...
bool DataSubstrate::InitializeMetrics(const INIReader &config_reader)
{
  ...
  DLOG(INFO) << "enable_metrics: " << (metrics::enable_metrics ? "ON" : "OFF");
  if (metrics::enable_metrics)
  {
    ...
    if (metrics::enable_tx_metrics)
    {
      metrics::collect_tx_duration_round=
          config_reader.GetInteger("metrics", "collect_tx_duration_round",
                                   metrics::collect_tx_duration_round);
    ...
    metrics::enable_busy_round_metrics=
        config_reader.GetBoolean("metrics", "enable_busy_round_metrics", true);
    ...
    metrics::enable_mysql_tx_metrics=
        config_reader.GetBoolean("metrics", "enable_mysql_tx_metrics", true);
    metrics::enable_mysql_dml_metrics=
        config_reader.GetBoolean("metrics", "enable_mysql_dml_metrics", true);
#endif
    ...
    {
      LOG(ERROR) << "!!!!!!!! Failed to initialize MetricsRegristry !!!!!!!!";
      return false;
    }

    metrics_registry_= std::move(metrics_registry_result.metrics_registry_);

    // TODO(liunyl): Finish all metrics registration before collect metrics.
    // We might need to finish all registration before starting tx service.
    if (core_config_.enable_data_store)
    {
    ...
    mysql_common_labels["node_port"]= std::to_string(network_config_.local_port);
